        ...
        <div className="flex-between flex-row md:flex-row space-x-2 space-y-2 mb-10">
            {['User Login', 'Shipping Address', 'Payment Method', 'Place Order'].map((step,index) =>(
                <React.Fragment key={step}>
                    <div className={cn('p-1 md:p-2 lg:p-2 w-48 md:w-56 lg:w-56 rounded-full text-center text-[10px] lg:text-sm md:text-xs border border-black', index === current ? 'bg-blue-100' : '' )}>{step}</div>
                    {step !== 'Place Order' && (
